  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={pitchData ? t('pitch_modal_edit_title') : t('pitch_modal_create_title')} size="xl">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label htmlFor="pitchType" className="block text-sm font-medium text-gray-700">{t('pitch_type_label')}</label>
        ...
        <div>
          <label htmlFor="pitchTitle" className="block text-sm font-medium text-gray-700">{t('pitch_title_label')}</label>
        ...
        <div>
          <label htmlFor="pitchTargetAudience" className="block text-sm font-medium text-gray-700">{t('pitch_target_audience_label')}</label>
        ...
        <div>
          <label htmlFor="pitchKeyMessage" className="block text-sm font-medium text-gray-700">{t('pitch_key_message_label')}</label>
        ...
        <div>
          <label htmlFor="pitchContent" className="block text-sm font-medium text-gray-700">{t('pitch_content_label')}</label>
          <textarea id="pitchContent" value={content} onChange={(e) => setContent(e.target.value)} rows={type === 'email_campaign' ? 8 : 5} required
                    className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm"
                    placeholder={t('pitch_content_placeholder')} />
           {type === 'email_campaign' && <p className="text-xs text-gray-500 mt-1">{language === 'am' ? 'ለኢሜል ዘመቻዎች፣ እያንዳንዱን ኢሜል በአዲስ መስመር ወይም በJSON ድርድር ቅርጸት ይለያዩ።' : 'For email campaigns, separate each email on a new line or as a JSON array if generated by AI.'}</p>}
        </div>
        <div>
          <label htmlFor="pitchNotes" className="block text-sm font-medium text-gray-700">{t('pitch_notes_label')}</label>
          <textarea id="pitchNotes" value={notes} onChange={(e) => setNotes(e.target.value)} rows={2}
                    className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm" />
        </div>
